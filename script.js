...
function handleCommandRaw(raw){
  ...
  switch(cmd){
    ...
    case'game':
      ...
      import('./game.js').then(g=>g.startGame(currentLevel)); // Richiamo gioco
      break;
...
document.addEventListener('click', ()=>{ if(adminOpen){ adminPassWrapper.style.display='none'; adminOpen=false; } });
...
toggleGameBtn.addEventListener('click', async ()=>{
  try{
    const res = await fetch('/api/updateGameState', {
      ...
      body:JSON.stringify({ password:'Seren1987', gameAvailable:true })
    });
    const data = await res.json();
    if(data.success){ gameActive=true; gameStatusSpan.textContent='Game: ON'; enqueueLine('> GAME ENABLED BY ADMIN', false, true);}
  }catch(e){ console.error('Errore updating game state', e); }
});
toggleGameOffBtn.addEventListener('click', async ()=>{
  try{
    const res = await fetch('/api/updateGameState', {
      method:'POST',
      headers:{'Content-Type':'application/json'},
      body:JSON.stringify({ password:'Seren1987', gameAvailable:false })
    });
    const data = await res.json();
    if(data.success){ gameActive=false; gameStatusSpan.textContent='Game: OFF'; enqueueLine('> GAME DISABLED BY ADMIN', false, true);}
  }catch(e){ console.error('Errore updating game state', e); }
});
