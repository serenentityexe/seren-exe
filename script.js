...
async function fetchGameState() {
  try {
    ...
    enqueueLine('> ERROR FETCHING GAME STATE', true, true);
  }
}
fetchGameState();
...
function updateGameStatusText() {
  ...
  if (span)
    span.textContent = gameActive
      ? 'Game: ON — SEREN ACTIVATED'
      : 'Game: OFF — SEREN DEACTIVATED';
}

async function saveUserData() {
  const userData = { level: currentLevel, progressData: gameProgress };
  try {
    await fetch(`${API_BASE}/saveUserData`, {
      method: 'POST',
...
async function loadUserData() {
  try {
    const res = await fetch(`${API_BASE}/getUserData?userId=${userId}`);
    const data = await res.json();
...
function handleCommandRaw(raw) {
  ...
  switch (cmd) {
    ...
    case 'game':
      if (!gameActive) {
        enqueueLine('> GAME NOT AVAILABLE. SEREN DEACTIVATED.', false, true);
        break;
      }
...
adminToggle.style.cssText =
  'position:absolute;top:10px;right:10px;width:16px;height:16px;border-radius:50%;background:var(--accent);cursor:pointer;z-index:20;';
...
toggleGameBtn.addEventListener('click', async () => {
  ...
  try {
    const res = await fetch(`${API_BASE}/updateGameState`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    if (data.success) {
      ...
      updateGameStatusText();
      enqueueLine(
        `> GAME STATE SET TO ${gameActive ? 'ON' : 'OFF'} BY ADMIN`,
        false,
        true
      );
    } else {
