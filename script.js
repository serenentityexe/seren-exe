...
async function startSystem(){
  await serenWrite("Avvio sistema SerenOS v1.0...");
  await serenWrite("[####..........] 25%");
  await serenWrite("[########......] 50%");
  await serenWrite("[############..] 90%");
...
async function handleCommand(cmd){
  ...
  switch(command){
    ...
    case "CLEAR":
      ...
      break;
    case "INFO":
      terminalOutput.innerHTML+=glitchText("SerenOS v1.0 - Terminale futuristico\nAutore: Seren\nFunzioni: Boot testuale, gioco, pannello admin\n");
      break;
    case "GAME":
...
terminalInput.addEventListener("keydown",(e)=>{
  if(e.key==="Enter"){
    ...
    terminalOutput.innerHTML+=`System/seren.exe@user ${cmd}\n`;
    terminalInput.value="";
    handleCommand(cmd);
  }
});
